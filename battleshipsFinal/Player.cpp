...
void Player::setTargeting(char arri, char inputValue) { // Jumps to coordX rows along + coordY in the row
    if (arri >= 0 &&
        arri < 100 &&
...
char Player::getTargeting(
    char arri) { // Jumps to coordX rows along + coordY in the row
  if (arri >= 0 && arri < 100)
  {
...
void Player::shootEnemy(Player &pEnemy, char arri)
{
  ...
  else if (shotOutcome >= ShipID::Destroyer &&
           shotOutcome <= ShipID::Carrier)
  { // Hit
